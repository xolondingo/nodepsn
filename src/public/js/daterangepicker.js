! function(t, a) {
    ...
    function i(t, e, a) {
        ...
        if (void 0 === e.startDate && void 0 === e.endDate && R(this.element).is(":text") && (o = i = null, 2 == (r = (n = R(this.element).val()).split(this.locale.separator)).length ? (o = j(r[0], this.locale.format), i = j(r[1], this.locale.format)) : this.singleDatePicker && "" !== n && (o = j(n, this.locale.format), i = j(n, this.locale.format)), null !== o && null !== i && (this.setStartDate(o), this.setEndDate(i))), "object" == typeof e.ranges) {
            for (s in e.ranges) {
                o = "string" == typeof e.ranges[s][0] ? j(e.ranges[s][0], this.locale.format) : j(e.ranges[s][0]), i = "string" == typeof e.ranges[s][1] ? j(e.ranges[s][1], this.locale.format) : j(e.ranges[s][1]), this.minDate && o.isBefore(this.minDate) && (o = this.minDate.clone());
                var l, c, d = this.maxDate;
                this.maxSpan && d && o.clone().add(this.maxSpan).isAfter(d) && (d = o.clone().add(this.maxSpan)), d && i.isAfter(d) && (i = d.clone()), this.minDate && i.isBefore(this.minDate, this.timepicker ? "minute" : "day") || d && o.isAfter(d, this.timepicker ? "minute" : "day") || ((l = document.createElement("textarea")).innerHTML = s, c = l.value, this.ranges[c] = [o, i])
            }
            var m = "<ul>";
    ...
    return i.prototype = {
        ...
        setStartDate: function(t) {
            "string" == typeof t && (this.startDate = j(t, this.locale.format)), "object" == typeof t && (this.startDate = j(t)), this.timePicker || (this.startDate = this.startDate.startOf("day")), this.timePicker && this.timePickerIncrement && this.startDate.minute(Math.round(this.startDate.minute() / this.timePickerIncrement) * this.timePickerIncrement), this.minDate && this.startDate.isBefore(this.minDate) && (this.startDate = this.minDate.clone(), this.timePicker && this.timePickerIncrement && this.startDate.minute(Math.round(this.startDate.minute() / this.timePickerIncrement) * this.timePickerIncrement)), this.maxDate && this.startDate.isAfter(this.maxDate) && (this.startDate = this.maxDate.clone(), this.timePicker && this.timePickerIncrement && this.startDate.minute(Math.floor(this.startDate.minute() / this.timePickerIncrement) * this.timePickerIncrement)), this.isShowing || this.updateElement(), this.updateMonthsInView()
        },
        setEndDate: function(t) {
            "string" == typeof t && (this.endDate = j(t, this.locale.format)), "object" == typeof t && (this.endDate = j(t)), this.timePicker || (this.endDate = this.endDate.endOf("day")), this.timePicker && this.timePickerIncrement && this.endDate.minute(Math.round(this.endDate.minute() / this.timePickerIncrement) * this.timePickerIncrement), this.endDate.isBefore(this.startDate) && (this.endDate = this.startDate.clone()), this.maxDate && this.endDate.isAfter(this.maxDate) && (this.endDate = this.maxDate.clone()), this.maxSpan && this.startDate.clone().add(this.maxSpan).isBefore(this.endDate) && (this.endDate = this.startDate.clone().add(this.maxSpan)), this.previousRightTime = this.endDate.clone(), this.container.find(".drp-selected").html(this.startDate.format(this.locale.format) + this.locale.separator + this.endDate.format(this.locale.format)), this.isShowing || this.updateElement(), this.updateMonthsInView()
        },
        ...
        renderCalendar: function(t) {
            ...
            var v, Y = this.locale.monthNames[e[1][1].month()] + e[1][1].format(" YYYY");
            if (this.showDropdowns) {
                for (var w = e[1][1].month(), P = e[1][1].year(), x = b && b.year() || this.maxYear, M = k && k.year() || this.minYear, I = P == M, S = P == x, B = '<select class="monthselect">', A = 0; A < 12; A++)(!I || k && A >= k.month()) && (!S || b && A <= b.month()) ? B += "<option value='" + A + "'" + (A === w ? " selected='selected'" : "") + ">" + this.locale.monthNames[A] + "</option>" : B += "<option value='" + A + "'" + (A === w ? " selected='selected'" : "") + " disabled='disabled'>" + this.locale.monthNames[A] + "</option>";
                B += "</select>";
                for (var L = '<select class="yearselect">', N = M; N <= x; N++) L += '<option value="' + N + '"' + (N === P ? ' selected="selected"' : "") + ">" + N + "</option>";
                Y = B + (L += "</select>")
            }
            C += '<th colspan="5" class="month">' + Y + "</th>", b && !b.isAfter(e.lastDay) || this.linkedCalendars && "right" != t && !this.singleDatePicker ? C += "<th></th>" : C += '<th class="next available"><span></span></th>', C += "</tr>", C += "<tr>", (this.showWeekNumbers || this.showISOWeekNumbers) && (C += '<th class="week">' + this.locale.weekLabel + "</th>"), R.each(this.locale.daysOfWeek, function(t, e) {
                C += "<th>" + e + "</th>"
            }), C += "</tr>", C += "</thead>", C += "<tbody>", null == this.endDate && this.maxSpan && (v = this.startDate.clone().add(this.maxSpan).endOf("day"), b && !v.isBefore(b) || (b = v));
            for (y = 0; y < 6; y++) {
                C += "<tr>", this.showWeekNumbers ? C += '<td class="week">' + e[y][0].week() + "</td>" : this.showISOWeekNumbers && (C += '<td class="week">' + e[y][0].isoWeek() + "</td>");
                for (g = 0; g < 7; g++) {
                    var E = [];
                    e[y][g].isSame(new Date, "day") && E.push("today"), 5 < e[y][g].isoWeekday() && E.push("weekend"), e[y][g].month() != e[1][1].month() && E.push("off", "ends"), this.minDate && e[y][g].isBefore(this.minDate, "day") && E.push("off", "disabled"), b && e[y][g].isAfter(b, "day") && E.push("off", "disabled"), this.isInvalidDate(e[y][g]) && E.push("off", "disabled"), e[y][g].format("YYYY-MM-DD") == this.startDate.format("YYYY-MM-DD") && E.push("active", "start-date"), null != this.endDate && e[y][g].format("YYYY-MM-DD") == this.endDate.format("YYYY-MM-DD") && E.push("active", "end-date"), null != this.endDate && e[y][g] > this.startDate && e[y][g] < this.endDate && E.push("in-range");
            ...
            if ("right" != t || this.endDate) {
                ...
                for (var o = this.timePicker24Hour ? 0 : 1, h = this.timePicker24Hour ? 23 : 12, l = o; l <= h; l++) {
                    ...
                    var d = a.clone().hour(c),
                        m = !1;
                    i && d.minute(59).isBefore(i) && (m = !0), r && d.minute(0).isAfter(r) && (m = !0), c != a.hour() || m ? e += m ? '<option value="' + l + '" disabled="disabled" class="disabled">' + l + "</option>" : '<option value="' + l + '">' + l + "</option>" : e += '<option value="' + l + '" selected="selected">' + l + "</option>"
                }
                ...
                for (var p, f, l = 0; l < 60; l += this.timePickerIncrement) {
                    var u = l < 10 ? "0" + l : l,
                        d = a.clone().minute(l),
                        m = !1;
                    i && d.second(59).isBefore(i) && (m = !0), r && d.second(0).isAfter(r) && (m = !0), a.minute() != l || m ? e += m ? '<option value="' + l + '" disabled="disabled" class="disabled">' + u + "</option>" : '<option value="' + l + '">' + u + "</option>" : e += '<option value="' + l + '" selected="selected">' + u + "</option>"
                }
                if (e += "</select> ", this.timePickerSeconds) {
                    e += ': <select class="secondselect">';
                    for (l = 0; l < 60; l++) {
                        u = l < 10 ? "0" + l : l, d = a.clone().second(l), m = !1;
                        i && d.isBefore(i) && (m = !0), r && d.isAfter(r) && (m = !0), a.second() != l || m ? e += m ? '<option value="' + l + '" disabled="disabled" class="disabled">' + u + "</option>" : '<option value="' + l + '">' + u + "</option>" : e += '<option value="' + l + '" selected="selected">' + u + "</option>"
                    }
                    e += "</select> "
                }
                this.timePicker24Hour || (e += '<select class="ampmselect">', f = p = "", i && a.clone().hour(12).minute(0).second(0).isBefore(i) && (p = ' disabled="disabled" class="disabled"'), r && a.clone().hour(0).minute(0).second(0).isAfter(r) && (f = ' disabled="disabled" class="disabled"'), 12 <= a.hour() ? e += '<option value="AM"' + p + '>AM</option><option value="PM" selected="selected"' + f + ">PM</option>" : e += '<option value="AM" selected="selected"' + p + '>AM</option><option value="PM"' + f + ">PM</option>", e += "</select>"), this.container.find(".drp-calendar." + t + " .calendar-time").html(e)
            }
        },
        ...
        move: function() {
            ...
            var s, n, r = this.container.outerWidth();
            this.container.toggleClass("drop-up", "up" == a), "left" == this.opens ? r + (s = i - this.element.offset().left - this.element.outerWidth()) > R(window).width() ? this.container.css({
                top: t,
                right: "auto",
                left: 9
            }) : this.container.css({
        ...
        clickRange: function(t) {
            var e, a = t.target.getAttribute("data-range-key");
        ...
        timeChanged: function(t) {
            ...
            var n, r, o, h = this.timePickerSeconds ? parseInt(e.find(".secondselect").val(), 10) : 0;
            this.timePicker24Hour || ("PM" === (n = e.find(".ampmselect").val()) && i < 12 && (i += 12), "AM" === n && 12 === i && (i = 0)), a ? ((r = this.startDate.clone()).hour(i), r.minute(s), r.second(h), this.setStartDate(r), this.singleDatePicker ? this.endDate = this.startDate.clone() : this.endDate && this.endDate.format("YYYY-MM-DD") == r.format("YYYY-MM-DD") && this.endDate.isBefore(r) && this.setEndDate(r.clone())) : this.endDate && ((o = this.endDate.clone()).hour(i), o.minute(s), o.second(h), this.setEndDate(o)), this.updateCalendars(), this.updateFormInputs(), this.renderTimePicker("left"), this.renderTimePicker("right")
        },
